...
### 점령 시스템 단순화
- **분할 화면 제거**: 테트리스 전체화면 + 우측 상단 미니 정보 패널
- **테트리스 진행 중**: 디펜스가 백그라운드에서 계속 진행
- **미니 패널 표시**: CORE HP, WAVE(강화 페이지), ENEMIES 수
- **테트리스 클리어**: 디펜스 화면 복귀, 강화 페이지 완료까지 대기
- **테트리스 실패**: 적 5마리 페널티 스폰, 디펜스 계속
