...
export class TetrisGame {
    constructor(containerId) {
        ...
        this.CONFIG = {
            ...
            COLORS: {
                I: 0x00f0ff, O: 0xfff000, T: 0xb026ff, S: 0x39ff14,
                Z: 0xff073a, J: 0x0044ff, L: 0xffa500,
                GHOST: 0x222222, GRID: 0x111111
            }
        };

        this.SPECIAL_TYPES = {
            NONE: 0, BOMB: 1, FREEZE: 2, LASER: 3, GOLD: 4
        };

        this.TETROMINOS = {
            I: { shape: [[1,1,1,1]], color: this.CONFIG.COLORS.I },
            O: { shape: [[1,1],[1,1]], color: this.CONFIG.COLORS.O },
            T: { shape: [[0,1,0],[1,1,1]], color: this.CONFIG.COLORS.T },
            S: { shape: [[0,1,1],[1,1,0]], color: this.CONFIG.COLORS.S },
            Z: { shape: [[1,1,0],[0,1,1]], color: this.CONFIG.COLORS.Z },
            J: { shape: [[1,0,0],[1,1,1]], color: this.CONFIG.COLORS.J },
            L: { shape: [[0,0,1],[1,1,1]], color: this.CONFIG.COLORS.L }
        };
    ...
    initTouchControls() {
        ...
        document.addEventListener("touchmove", e => {
            ...
            if (Math.abs(diffX) > this.CONFIG.SWIPE_SENSITIVITY) {
                const steps = Math.floor(Math.abs(diffX) / this.CONFIG.SWIPE_SENSITIVITY);
                if (steps > 0) {
    ...
    initThree() {
        const CELL_HEIGHT = (2 * Math.PI * this.CONFIG.RADIUS) / this.CONFIG.GRID_WIDTH;
    ...
    initMaterials() {
        ...
        colorKeys.forEach(key => {
            ...
            this.sharedMaterials[color] = new THREE.MeshStandardMaterial({
                ...
                transparent: true, opacity: 1.0, roughness: 0.2, metalness: 0.1
            });
            this.sharedMaterials[color + '_ghost'] = new THREE.MeshStandardMaterial({
                color: color, emissive: color, emissiveIntensity: 0.3,
                transparent: true, opacity: 0.3, roughness: 0.2, metalness: 0.1
            });
        ...
        Object.keys(this.SPECIAL_TYPES).forEach(key => {
            ...
            switch(type) {
                case this.SPECIAL_TYPES.BOMB: color = 0xff0000; emissive = 0xff0000; break;
                case this.SPECIAL_TYPES.FREEZE: color = 0x00ffff; emissive = 0x00ffff; break;
                case this.SPECIAL_TYPES.LASER: color = 0xffff00; emissive = 0xffff00; break;
                case this.SPECIAL_TYPES.GOLD: color = 0xffd700; emissive = 0xffd700; break;
            }
    ...
    createCylinderGrid(CELL_HEIGHT, TOTAL_HEIGHT) {
        ...
        for (let y = 0; y <= this.CONFIG.GRID_HEIGHT; y++) {
            const curve = new THREE.EllipseCurve(
                0, 0,
                this.CONFIG.RADIUS - 0.1, this.CONFIG.RADIUS - 0.1,
                0,  2 * Math.PI,
                false,
                0
            );
    ...
    startGame() {
        this.state.grid = Array(this.CONFIG.GRID_HEIGHT).fill().map(() => Array(this.CONFIG.GRID_WIDTH).fill(null));
        this.state.score = 0;
        this.state.level = 1;
    ...
    drawNextPiece() {
        ...
        for (let r = 0; r < shape.length; r++) {
            for (let c = 0; c < shape[r].length; c++) {
                if (shape[r][c]) {
                    if (blockCount === specialIndex && specialType !== this.SPECIAL_TYPES.NONE) {
    ...
    spawnPiece() {
        ...
        this.state.currentPiece = {
            ...
            shape: template.shape,
            color: template.color,
            x: Math.floor(this.CONFIG.GRID_WIDTH / 2) - Math.floor(template.shape[0].length / 2),
            y: this.CONFIG.GRID_HEIGHT - 1 - template.shape.length,
            specialType: template.specialType || this.SPECIAL_TYPES.NONE,
            specialIndex: (template.specialIndex !== undefined) ? template.specialIndex : -1
        };
        ...
        const centerAngle = (this.state.currentPiece.x / this.CONFIG.GRID_WIDTH) * Math.PI * 2;
    ...
    checkCollision(px, py, shape) {
        for (let r = 0; r < shape.length; r++) {
            for (let c = 0; c < shape[r].length; c++) {
                if (shape[r][c]) {
                    const testX = (px + c + this.CONFIG.GRID_WIDTH * 10) % this.CONFIG.GRID_WIDTH;
                    const testY = py - r;
                    if (testY < 0) return true;
                    if (testY < this.CONFIG.GRID_HEIGHT && this.state.grid[testY][testX]) return true;
                }
            }
        }
        return false;
    ...
    lockPiece() {
        ...
        for (let r = 0; r < p.shape.length; r++) {
            for (let c = 0; c < p.shape[r].length; c++) {
                if (p.shape[r][c]) {
                    const x = (p.x + c + this.CONFIG.GRID_WIDTH * 10) % this.CONFIG.GRID_WIDTH;
                    const y = p.y - r;
                ...
                if (effect.type === this.SPECIAL_TYPES.BOMB) {
                    for (let dy = -1; dy <= 1; dy++) {
                        for (let dx = -1; dx <= 1; dx++) {
                            ...
                            const targetX = (effect.x + dx + this.CONFIG.GRID_WIDTH * 10) % this.CONFIG.GRID_WIDTH;
                    ...
                    if(!this.state.originalSpeed) this.state.originalSpeed = this.CONFIG.DROP_SPEED;
    ...
    updateCamera() {
        ...
        const target = ((this.state.currentPiece.x + pieceCenterOffset) / this.CONFIG.GRID_WIDTH) * Math.PI * 2;
    ...
    renderActivePiece() {
        ...
        if (this.CONFIG.SHOW_GHOST) {
            ...
            while (!this.checkCollision(this.state.currentPiece.x, ghostY - 1, this.state.currentPiece.shape)) {
                ghostY--;
            }
            ...
            for (let r = 0; r < p.shape.length; r++) {
                for (let c = 0; c < p.shape[r].length; c++) {
                    if (p.shape[r][c]) {
                        const gx = (p.x + c + this.CONFIG.GRID_WIDTH * 10) % this.CONFIG.GRID_WIDTH;
                        const gy = ghostY - r;
        ...
        for (let r = 0; r < p.shape.length; r++) {
            for (let c = 0; c < p.shape[r].length; c++) {
                if (p.shape[r][c]) {
                    ...
                    this.addBlockToGroup(gx, gy, { color: p.color, type: blockType }, activeGroup);
                    blockCount++;
                }
            }
        }
    ...
    addBlockToGroup(gx, gy, blockInfo, group, isGhost = false) {
        ...
        if (isGhost) {
            ...
            mat = this.CONFIG.TRANSPARENT_MODE ? this.sharedMaterials[color + '_trans'] : this.sharedMaterials[color];
        }
